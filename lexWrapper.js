...
const debug = require("debug")("lexWrapper.js");
...
let lexer = new Lexer(function (char) {
    let errm = `Unexpected character at row ${row}, col ${col}: '${char}'`;
    // debug(errm);
...
function parseSelection(selection, token) {
    ...
    let NLNO = (selection.match(/\n/g) || []).length;
    row += NLNO;

    // Set the column counter as the number of characters from the last newline
    if (NLNO === 0)
        col += selection.length;
    else
        col += selection.length - selection.lastIndexOf("\n");
...
//     row++;
//     col = 1;
//     pointer += NLI.length; // NLI should be either /n, either /r/n
// }, []);
